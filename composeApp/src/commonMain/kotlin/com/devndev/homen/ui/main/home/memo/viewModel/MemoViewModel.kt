...
class MemoViewModel(
    ...
    override fun handleEvents(event: MemoContract.Event) {
        when (event) {
            is MemoContract.Event.OnSaveClick -> {
                when (event.isEdit) {
                    true -> {
                        editMemo(choreId = event.choreId, memoId = event.memoId!!)
                    }
                    false -> {
    ...
    private fun editMemo(choreId: Int, memoId: Int) {
        viewModelScope.launch {
            val result = editMemoUseCase(choreId = choreId, memoId = memoId ,content = viewState.value.content)
            when (result) {
                is ApiResult.Success -> {
                    setEffect { MemoContract.Effect.NavigateToBack }
                }
